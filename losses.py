...
def compute_losses(
    ...
    # 关键：这项作为 loss 的 target，只让 cw 收梯度，不让 cw_budget 这边往回传
    cw_gap    = cw  - cw_budget.detach()          # 自给状态预算残差（target 已 detach）

    # ---- Default indicator for HJB (1{v < w})
    # 用 detach 不让 default_mask 把梯度链回 v,w
    default_mask = (v.detach() < w.detach()).float()  # 违约区域指示函数
    ...
    # 这里故意不 detach：BSDE target 的梯度已经在 v_hat/w_hat/q_hat 上切断
    ...
    # BSDE losses（target 一侧已完全 detach）
    ...
    # 注意：这里用 v.detach()/w.detach()/q.detach()，不让 Dv,Dw,Dq 把梯度链回去
    Dv = (A_V - v.detach().view(N, 1, 1)) / Delta
    Dw = (A_W - w.detach().view(N, 1, 1)) / Delta
    Dq = (A_Q - q.detach().view(N, 1, 1)) / Delta

    Dv = Dv.view(N, 1)
    Dw = Dw.view(N, 1)
    Dq = Dq.view(N, 1)

    # value at b=0 for v(0,k,s,z) in Loss_w —— target 端需要 detach
    states_b0 = states.clone()
    states_b0[:, 0:1] = 0.0
    out_b0 = net(states_b0)
    v_b0 = out_b0["v"]  # 作为 target，不给 v 网络梯度

    # PDE 用的 utility 也用 detach(c/cw)，不让 target 端更新 c/cw
    u_level_g_pde = utility_level(c.detach(),  l,   cfg)
    u_level_w_pde = utility_level(cw.detach(), l_w, cfg)
    ...
    # 上面已经把 cw_gap 写成 cw - cw_budget.detach()，所以这里直接用它即可
    ...
    uc_val = uc(c, l_det, cfg)                # 对 c 仍有梯度，对 l_det 没有
...
#     cw_budget = y_w          - phi_adj_w
#     # 关键：cw_budget 作为 target，不让这条链路反向到 W_k 等
#     cw_gap    = cw  - cw_budget.detach()          # 自给状态预算残差

#     # ---- Default indicator for HJB (1{v < w})
#     default_mask = (v.detach() < w.detach()).float()  # 违约区域指示函数（不传梯度）
...
#     # Instantaneous utility (good / autarky) for BSDE part（可以保留梯度）
#     u_level_g = utility_level(c,  l,   cfg)   # [N,1]
#     u_level_w = utility_level(cw, l_w, cfg)   # [N,1]
...
#     # ============================================================
#     # Step 5: Analytic PDE / HJB losses —— 用“等式残差”形式
#     # ============================================================
#     # D-terms are discrete-time generators:
#     #   D(x) = ( A(next_x) - x ) / Delta, where A is average over h paths.
#     A_V = V_next.mean(dim=1, keepdim=True)   # [N,1,1]
...
#     # 这里不对 v/w/q detach，让 D-term 真正感受到它们的尺度；等式残差本身会惩罚爆炸
...
#     euler_v_res = (rho + gamma * pi) * v - (u_level_g_pde + Dv + gamma * pi * w)
...
#     # 为了 metrics，额外构造一份“解出来的 target_v/w/q”（只用于监控，no_grad 即可）
#     with torch.no_grad():
#         denom_w = rho + lambda_b
#         denom_v = rho + gamma * pi
#         denom_q = rho + gamma * pi
...
#     # 3) PDE “目标”（其实是 HJB 方程右侧除以 discount）
#     metrics.update(_stats_quantiles(target_v,  "pde_target_v"))
#     metrics.update(_stats_quantiles(target_w,  "pde_target_w"))
#     metrics.update(_stats_quantiles(target_q,  "pde_target_q"))
...
#     # ============================================================
#     # Step 5: Analytic PDE / HJB losses —— 用等式残差形式
#     # ============================================================
#     # D-terms are discrete-time generators:
#     #   D(x) = ( A(next_x) - x ) / Delta, where A is average over h paths.
#     A_V = V_next.mean(dim=1, keepdim=True)   # [N,1,1]
...
#     loss_pde = loss_w_pde + 10*loss_v_pde + loss_q_pde
...
#     w_pde    = cfg["train"]["loss_weights"]["pde"]
#     w_policy = cfg["train"]["loss_weights"]["policy"]
#     # corner 权重：如果 config 里没配，就默认 0
#     lw = cfg["train"]["loss_weights"]
#     w_corner = lw["corner"] if "corner" in lw else 0.0
