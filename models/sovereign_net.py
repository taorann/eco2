...
# Inputs: x = [b, k, s, z].
# Heads:
#   Policy branch (independent params):
#       - c      : consumption in good standing  (bounded in (0.5, 1.5))
#       - cw     : consumption in autarky        (bounded in (0.5, 1.5))
#   Value branch:
#       - q      : long-term bond price          (0 < q < max_q via sigmoid)
#       - v      : value in good standing        (bounded via tanh)
#       - w      : value in autarky              (bounded via tanh)
#       - sigma_g: diffusion loading of q on W0  (positive via softplus)
# ---------------------------------------------------------
...
class SovereignNet(nn.Module):
    ...
    def __init__(
        ...
        self.head_q = SigmoidBoundedHead(val_good_dim, 1, max_val=max_q)
        self.head_sigma = PositiveHead(val_good_dim, 1, min_val=min_positive)
        ...
        # c, cw 也先用 (0, max_q) 的输出，再线性变换到 (0.5, 1.5)
        self.head_c = SigmoidBoundedHead(pol_good_dim, 1, max_val=max_q)
        self.head_cw = SigmoidBoundedHead(pol_aut_dim, 1, max_val=max_q)
